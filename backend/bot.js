...
bot.onText(/\/start(.*)/, async (msg, match) => {
  ...
  try {
    ...
    if (param) {
      ...
      bot.sendMessage(msg.chat.id,
        '✅ Привет, ' + user.name + `!\n\nДля входа нажмите "Войти через Telegram" на сайте ${SITE_URL}`
      );
    }
  } catch (err) {
...
bot.on('message', (msg) => {
  if (msg.text && !msg.text.startsWith('/')) {
    bot.sendMessage(msg.chat.id, `Для входа в систему нажмите "Войти через Telegram" на сайте ${SITE_URL}`);
  }
});

console.log('🤖 Doorium Telegram Bot started');
