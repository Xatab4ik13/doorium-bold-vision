...
const AdminRequests = () => {
  ...
  const handleCreate = async (data: Partial<ApiRequest>) => {
    const newReq: ApiRequest = {
      id: `r${Date.now()}`,
      number: `REQ-${String(requests.length + 1).padStart(3, "0")}`,
      ...
      interior_doors: data.interior_doors,
      entrance_doors: data.entrance_doors,
      source: "site",
        ...
        <Card className="bg-white border-slate-200 overflow-hidden">
          {isMobile ? (
            <div className="p-3 space-y-2">
              {paged.map((r, i) => (
                <MobileRequestCard key={r.id} request={r} index={i} onClick={() => setSelectedRequest(r)} getUserName={getDemoUserName} />
              ))}
                  ...
                  {paged.map((r) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-xs">
                        {r.partner_id ? (
                          <span className="px-1.5 py-0.5 bg-green-50 text-green-700 rounded text-xs">{getDemoUserName(r.partner_id) || "Партнёр"}</span>
                        ) : (
                          <span className="text-slate-400">Сайт</span>
                        )}
                      </td>
                      <td className="px-4 py-3 text-xs text-slate-600">{getDemoUserName(r.measurer_id) || getDemoUserName(r.installer_id) || "—"}</td>
